...
int main(void){
	uint32_t i;
	uint32_t delay;
	delay = 10;// (in us unit) keep a very short delay so that you don't have to wait for the led to toggle after pressing the button
  ...
  // Dead loop & program hangs here
	while(1)
		...
		// button press -IDR value at position 13 is set to 1 so check this bit to toggle
    if((GPIOC -> IDR & GPIO_IDR_IDR_13))
			{ 
				toggle_LED();// button press=toggle state of LED
				
				while((GPIOC -> IDR & GPIO_IDR_IDR_13))
					{
					 for(i=0; i<delay; i++); // fixing debouncing with software
					}
			}
